...
namespace Steamless.API.PE64
{
    ...
    public class Pe64Helpers
    {
        ...
        private static uint ComputePeChecksum(byte[] data)
        {
            ...
            // Sum all 16-bit words, folding overflow into the low 16 bits after each addition..
            ...
            // Handle odd-length files: treat last byte as high byte of a 16-bit word..
            if ((data.Length & 1) != 0)
            {
            ...
            // Fold the upper half into the lower half until it fits in 16 bits..
            checksum = (checksum & 0xFFFF) + (checksum >> 16);
            checksum = (checksum & 0xFFFF) + (checksum >> 16);
        ...
        public static bool UpdateFileChecksum(string path)
        {
            try
            {
                ...
                var checksumOffset = sigOffset + 4 +
                    (uint)Marshal.SizeOf(typeof(NativeApi64.ImageFileHeader64)) +
                    (uint)Marshal.OffsetOf(typeof(NativeApi64.ImageOptionalHeader64), "CheckSum").ToInt32();
